...
int server_setup() {
  int n = mkfifo(WKP, 0644);
  printf("Created WKP, Waiting for connection\n");
...
int server_connect(int from_client) {
  ...
  printf("Read Secret Pipe from Client\n");
  ...
  read(from_client,buf2,HANDSHAKE_BUFFER_SIZE);
  printf("read response from client\n");
  return to_client;
}
